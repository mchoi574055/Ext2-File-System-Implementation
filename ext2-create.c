...
void write_block_bitmap(int fd) {
	...
	u8 hexToWrite = 0xFF;
	ssize_t size = sizeof(hexToWrite);
	
	if (write(fd, &hexToWrite, size) != size) {
		errno_exit("write");
	}
...
void write_inode_table(int fd) {
	...
	struct ext2_inode helloworld_inode = {0};
	 helloworld_inode.i_mode = EXT2_S_IFREG
	                              | EXT2_S_IRUSR
	                              | EXT2_S_IWUSR
	                              | EXT2_S_IXUSR
	                              | EXT2_S_IRGRP
	...
	helloworld_inode.i_uid = 1000;
	helloworld_inode.i_size = 12;
	...
	helloworld_inode.i_dtime = 0;
        helloworld_inode.i_gid = 1000;
        helloworld_inode.i_links_count = 1;
	helloworld_inode.i_blocks = 2; /* These are oddly 512 blocks */
